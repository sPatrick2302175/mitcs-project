<x-app-layout>
    ...
    <div class="py-12 bg-gray-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Employee Search & Add Action Card -->
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100/60 p-5 mb-6 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
                <form id="search-form" method="GET" action="{{ route('employees.index') }}" onsubmit="event.preventDefault();" class="w-full max-w-md">
                    <label class="block text-[10px] font-bold uppercase tracking-wider text-gray-500 mb-2">Search Employee</label>
                    <div class="relative">
                        <input type="text" id="search-input" name="search" value="{{ request('search') }}" 
                            placeholder="Search by name or employee ID..." 
                            class="w-full bg-gray-50 border border-gray-200 text-sm rounded-xl px-4 py-2.5 focus:bg-white focus:border-[#F2A455] focus:ring-2 focus:ring-[#F2A455]/20 transition-all shadow-sm">
                        
                        <div id="table-spinner" class="hidden absolute right-3 top-3">
                            <svg class="animate-spin h-5 w-5 text-[#F2A455]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <a href="{{ route('employees.create') }}" 
                    class="inline-flex items-center justify-center px-5 py-2.5 h-[42px] bg-[#F2A455] border border-transparent rounded-xl font-bold text-xs text-white uppercase tracking-widest hover:bg-[#df9344] focus:outline-none focus:ring-2 focus:ring-[#F2A455]/40 active:scale-[0.98] transition-all duration-200 shadow-md shadow-orange-500/20 whitespace-nowrap">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                    Add New Employee
                </a>
            </div>

            <!-- Mass Allocate Action -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchFilteredData() {
                ...
                const fetchUrl = `${window.location.pathname}?${queryParams}`;
                ...
                fetch(fetchUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                    ...
                    .then(htmlString => {
                        ...
                        const freshTableContent = freshDocument.getElementById('employee-table-container');
