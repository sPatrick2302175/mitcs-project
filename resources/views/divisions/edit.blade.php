<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-white leading-tight">
            {{ __('Edit Division') }}: <span class="font-light">{{ $division->division_name }}</span>
        </h2>
    </x-slot>
    ...
    <div class="py-12 bg-gray-50/50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100/60 p-8 md:p-10 transition-all duration-300">
                ...
                <form action="{{ route('divisions.update', $division->id) }}" method="POST" class="space-y-8">
                    @csrf 
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="md:col-span-1 group">
                            ...
                            <input type="text" name="division_name" value="{{ old('division_name', $division->division_name) }}" required 
                        ...
                        <div class="md:col-span-2 group">
                            ...
                            <select name="department_id" required 
                                ...
                                @foreach($departments as $dept)
                                    <option value="{{ $dept->id }}" @selected(old('department_id', $division->department_id) == $dept->id)>
