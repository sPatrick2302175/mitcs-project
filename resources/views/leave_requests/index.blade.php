<x-app-layout>
    ...
    <style>
        ...
        .fc .fc-button-primary:not(:disabled):hover { background-color: #111827 !important; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
        ...
        .fc-event { border: none !important; border-radius: 0.5rem !important; padding: 0.15rem 0.25rem; font-size: 0.7rem; font-weight: 700; cursor: pointer; box-shadow: 0 1px 2px 0 rgba(0,0,0,0.05); transition: transform 0.1s ease; }
    ...
    <div class="py-12 bg-gray-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="bg-white border border-gray-100/60 rounded-2xl shadow-sm p-6 relative overflow-hidden group">
            ...
            <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100/60 overflow-hidden">
                <div class="p-6 md:p-8 border-b border-gray-100/60 flex flex-col md:flex-row justify-between items-center gap-4">
                    <h3 class="text-xl font-black text-gray-800 tracking-tight">My Leave Applications History</h3>
                    <a href="{{ route('leave-requests.create') }}" class="inline-flex items-center px-5 py-2.5 bg-[#F2A455] hover:bg-[#df9344] text-white text-xs font-extrabold uppercase tracking-wider rounded-xl shadow-md transition-all">
                                    ...
                                    <td class="py-4 px-6 whitespace-nowrap">
                                        ...
                                        @elseif($request->status === 'disapproved')
                                            <span class="inline-flex items-center px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-lg bg-rose-50 text-rose-600 border border-rose-100/60 shadow-sm">
                                                <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>Disapproved
                                            </span>
                                        @endif
                                    </td>
            ...
            <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100/60 overflow-hidden p-6 md:p-8">
                <div class="mb-6 flex justify-between items-start">
                    ...
                    @if (Auth::user()->is_admin >= App\Models\User::ROLE_DEPT_ADMIN)
                        <a href="{{ route('admin.custom-holidays.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white text-xs font-extrabold uppercase tracking-wider rounded-xl shadow-md transition-all">
                            <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                            Add Holiday
                        </a>
                    @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                // When an Admin clicks on an event:
                eventClick: function(info) {
                    var type = info.event.extendedProps.type;
                    var isAdmin = {{ Auth::user()->is_admin >= 1 ? 'true' : 'false' }};

                    // Click on a pending or approved leave goes to Review View
                    if (type === 'pending_leave' || type === 'approved_leave') {
                        if (isAdmin) {
                            window.location.href = '/admin/leave-requests/' + info.event.extendedProps.leave_id + '/review';
                        }
                    } 
                    // Click on a regular or custom holiday prompts Admin to manage it
                    else if (type === 'regular_holiday' || type === 'custom_holiday') {
                        if (isAdmin) {
                            if (confirm('Do you want to edit or move this holiday?\n\n(Clicking OK will take you to the Manage Holidays page where you can delete this date and add a new custom one.)')) {
                                window.location.href = "{{ route('admin.custom-holidays.index') }}";
                            }
                        }
                    }
                }
